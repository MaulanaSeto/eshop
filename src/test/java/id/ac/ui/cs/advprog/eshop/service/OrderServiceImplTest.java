...
class OrderServiceImplTest {
    ...
    void testFindAllByAuthorIfAuthorCorrect() {
        Order order = orders.get(1);
        doReturn(List.of(order, order)).when(orderRepository).findAllByAuthor(order.getAuthor());

        List<Order> results = orderService.findAllByAuthor(order.getAuthor());
        for (Order result : results) assertEquals(order.getAuthor(), result.getAuthor());
